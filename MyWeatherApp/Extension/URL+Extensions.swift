...
extension URL {
    ...
    static func urlForWeatherFor( latitude: String,longitude : String) -> URL? {
        ...
        guard let url = URL(string: "http://api.openweathermap.org/data/2.5/weather?lat=\(latitude)&lon=\(longitude)&appid=\(Constant.APIKEY)") else {
            return nil
        }
    ...
    static func urlForDailyWeatherFor( latitude: String,longitude : String) -> URL? {
        ...
        guard let url = URL(string: "http://api.openweathermap.org/data/2.5/forecast?lat=\(latitude)&lon=\(longitude)&appid=\(Constant.APIKEY)") else {
            return nil
        }
